...
addonID = 'plugin.video.youtube.movies'
...
def index():
    ...
    addDir(match[0], mainChannelID, "listVideosMain", icon)
...
def listVideos(url):
    ...
    content = getUrl(url)
    jsonContent = json.loads(content)["content_html"].replace("\\n","").replace("\\","")
