...
def listVideos(url):
    ...
    if 'video item"' in content:
        spl = content.split('video item"')
    elif 'class="thumb_image"' in content:
        spl = content.split('class="thumb_image"')
    for i in range(1, len(spl), 1):
        entry = spl[i]
        match = re.compile('class="name">(.+?)<', re.DOTALL).findall(entry)
        title = match[0]
    ...
    matchPage1 = re.compile('class="item next_page" rel="next" href="(.+?)"', re.DOTALL).findall(content)
    matchPage2 = re.compile('<div class="center">.*?<a href="(.+?)"', re.DOTALL).findall(content)
    if matchPage1:
        urlNext = "http://mpora.com"+matchPage1[0].replace("&amp;", "&")
        addDir(translation(30001), urlNext, 'listVideos', "")
    elif matchPage2:
        urlNext = "http://mpora.com"+matchPage2[0].replace("&amp;", "&")
        addDir(translation(30001), urlNext, 'listVideos', "")
    xbmcplugin.endOfDirectory(pluginhandle)
    if forceViewMode:
        xbmc.executebuiltin('Container.SetViewMode('+viewMode+')')
...
def getUrl(url):
    ...
    req.add_header('Cookie', 'locale=en')
