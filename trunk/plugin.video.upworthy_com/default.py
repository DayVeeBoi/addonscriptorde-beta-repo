...
def listVideos(url):
    ...
    for i in range(1, len(spl), 1):
        ...
        url = baseUrl+match[0]
        match = re.compile('<time datetime=".+?" pubdate>(.+?)</time>', re.DOTALL).findall(entry)
        date = match[0]
...
def listLatestVideos():
    ...
    for i in range(1, len(spl), 1):
        ...
        thumb = match[0].replace("//","http://")
        #Not working on all thumbs
        #thumb = match[0].replace("//","http://").replace("/rss_77","/web_600")
        ...
        if matchYoutube:
            addLink(title, matchYoutube[0], 'playYoutubeVideo', thumb, pubDate+"\n"+description)
        elif matchVimeo:
            addLink(title, matchVimeo[0], 'playVimeoVideo', thumb, pubDate+"\n"+description)
    xbmcplugin.endOfDirectory(pluginhandle)
    if forceViewMode:
        xbmc.executebuiltin('Container.SetViewMode('+viewMode+')')
